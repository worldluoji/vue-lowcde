<template>
  <div>
    ...
    <div v-if="radio === 'LowCode'">
      ...
      <el-input v-model="url" placeholder="请输入请求url" @change="change" />
    </div>
    <div v-else-if="radio === 'ProCode'">
      <el-button text @click="drawVisible = true"> 点我编辑高代码 </el-button>
      <el-drawer
        v-model="drawVisible"
        title="JavaScript编辑器"
        direction="ttb"
        :append-to-body="true"
      >
        <JSEditor :model-value="modelValue" @update:modelValue="change" />
      </el-drawer>
    </div>
  </div>
</template>
