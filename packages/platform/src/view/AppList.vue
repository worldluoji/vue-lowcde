        ...
        <el-main>
            <div class="app-list">
                <div v-for="app in appList" :key="app.id">
                    <el-card shadow="hover" @click="toPageList(app.id)"> {{  app.name }} </el-card>
                </div>
            </div>
            <div class="newApp">
                ...
                <el-dialog v-model="dialogNewAppVisible" title="新建应用">
                    ...
                    <span class="dialog-footer">
                        ...
                        <el-button type="primary" @click="addNewApp">
                            确定
                        </el-button>
                    </span>
                    </template>
...
const addNewApp = async () => {
    const res = await $request.post('/v1/app/create', {name: form.name, desc: form.desc})
    console.log(res);
    if (res.id) {
        ElMessage({
            message: '添加成功',
